...
>
> **Versão atual:** `v1.39.1` · 2026-07-29
>
> **v1.39.1 — Dashboard: polimento (impeccable):** **cor invertida corrigida** — "Etapas atrasadas" não passava `tone` e `kpi()` pinta de verde qualquer número `> 0`, então o card ficava verde quanto mais atraso houvesse (o alerta da mesma métrica já usava `warning`: card e alerta se contradiziam). **Percentuais** chegam ao `kpi()` como STRING formatada, logo `numeric` era `null` e o tom nunca aplicava — helper novo **`kpiToneNumero(n)`** deriva o tom do número antes de formatar (margem negativa = prejuízo; execução > 100% = estouro). **Estado vazio** no `lucroCaixaPanel`: sem lançamento, `lucroCaixaResolvePeriod` inventava período de 1 dia e mostrava 9 cards zerados. **`renderDashboard()` ganhou try/catch** (era a única tela crítica sem) — corpo movido para `renderDashboardBody()` para não reindentar 190 linhas. **Acessibilidade:** `chartPanel()` injeta o título no `aria-label` do SVG (havia dois "Gráfico de linha" na mesma tela); `<th scope="col">` no `table()` genérico. Teste novo `test_dashboard_polish.js` (20) — **primeiro teste do Dashboard**. Suíte: 12/12. **NÃO aplicado, aguarda decisão:** conflito filtro global × seletor do dashboard (zera a tela sem avisar), widget de execução ignorando a visão por obra, ordem dos blocos, O(n²) em `dashboardCostCenterRows`/`resultByProjectRows`, e `kpi()` colorindo contagens neutras de verde.
>
> **v1.39.0 — Kanban: visão "Todos os boards" + seletor agrupado por obra:** o seletor de board passa a agrupar por obra via `<optgroup>` (`kanbanBoardsPorObra()`; grupo "Sem obra vinculada" sempre por último). Opção nova **`KANBAN_TODOS`** (`"__todos__"`) consolida os cards de todas as obras agrupados pelo **NOME** da coluna — não pelo id, porque "A fazer" existe em cada board. Nessa visão cada card mostra etiqueta de obra e de board (`kanbanCardHtml(card, editable, mostrarOrigem=true)`) e **`draggable=false` de propósito**: com grupos por nome de coluna, o drop não definiria o board de destino. Filtros `kanbanFiltros {obra, responsavel, prioridade}` combinam em E; o filtro de obra usa **o board dono da coluna** (`kanbanCardsFiltrados()`), mesma fonte de verdade da validação de coerência da v1.38.5 — filtro e validação nunca discordam. Sem migration (reusa `kanban_boards.obra_id`). Teste `test_kanban_coerencia.js` = 24 asserções. Suíte: 11/11.
>
> **v1.38.5 — Card do Kanban nascia no board errado + `rowLabel()` (rótulo dos selects):** o card salvava mas sumia da tela (ia para board de outra obra). **Causa raiz: DUAS cadeias de fallback divergentes** — `nameOf()` (exibição) reconhecia `row.nome`, o `<select>` de FK em `inputFor()` NÃO. Como `kanban_colunas` usa `nome` (pt), o formulário mostrava **o ID cru**, e ainda listava colunas de todos os boards sem separação. **Fix estrutural: `rowLabel(row)`** virou fonte única, usada por `nameOf()` **e** pelo select — **ao criar entidade com o rótulo noutro campo, acrescente em `rowLabel` e os dois lados seguem juntos**. Alcance: **12 tabelas** com `nome` em pt (obra_disciplinas, cotacao_categorias/tipos_item, rh_colaboradores/tipos_documento, orcamento_etapas, proposta_grupos/modelos, viabilidade_analises/grupos, kanban_boards/colunas) exibiam ID ao editar. Select de coluna agora usa `<optgroup>` "Board (Obra)". **`normalizeKanbanCard()` mudou de contrato: devolve string de erro** (padrão de `normalizeViabilityAnalysis`) e bloqueia salvar quando a obra do card diverge da obra do board dono da coluna; sem obra informada, herda a do board. Teste novo `test_kanban_coerencia.js` (14). Suíte: 11/11. **Dados legados:** cards já gravados incoerentes NÃO foram tocados — decisão do dono.
>
> **v1.38.4 — 500 do Kanban (SQLSTATE 22003) + blindagem da classe 22xxx + log com destino:** criar/arrastar card dava 500 porque `kanban_cards.ordem` é **INT** (máx 2.147.483.647) e o front gravava `Date.now()` (ms, ~1,78 trilhão — 831× o limite). Estourava com **22003**, que está FORA de 23000 e não era convertido → 500 genérico. Contraprova: a automação de pedido usa `time()` no PHP (segundos, cabe) e sempre funcionou. **Fix:** helper **`kanbanOrdemAgora()`** = `Math.floor(Date.now()/1000)` nos 4 pontos (`openForm`, `normalizeKanbanCard`, `moveKanbanCard`, `createLocalPurchaseKanbanCard` — este porque a Migração leva o local ao banco). **REGRA: nunca grave `ordem` com `Date.now()`** — há teste que trava (`test_kanban_ordem.js`). **Blindagem geral:** função pura nova **`sql_error_response($sqlstate, $context)`** (usada por `fail_if_integrity_violation`) traduz 23000→409 e agora **22xxx→422** (22003 limite, 22007/22008 data, 22001 texto longo, resto genérico). Infra (42S02/42S22/conexão) **continua subindo como 500 de propósito** — é problema nosso, tem de aparecer. **Log:** havia `log_errors=1` mas **nenhum `error_log` definido** — sob PHP-FPM o erro ia para o log do pool e o Apache ficava vazio (foi o que travou este diagnóstico); agora aponta para `/var/lib/financeiro/logs/php-error.log` se o dir existir e for gravável. Testes novos: `test_sql_error_response.php` (27) e `test_kanban_ordem.js` (6). Suíte: 10/10. **Pendente (ciclo próprio):** `ordem` como sequencial `MAX+1` ou coluna → BIGINT (o INT estoura em 2038).
>
> **v1.38.3 — Modo privacidade Etapa 3 (cobertura total) — MÓDULO CONCLUÍDO:** os 163 `asMoney` classificados um a um: **104 → `moneySpan()`** (HTML de tela: Custo da Obra/execução/visões, telas de IA, Cotações por material e Resultado, Compras/matriz, Centros de custo, Curva ABC, OFX, prévia NFS-e, Viabilidade financeira, cronograma, painel de grupos da proposta, modais de parcelas/quitação/gerar conta/anexar NF); **18 → `maskMoneyText()`**; **38 intocados** (documento/export). **REGRA PARA CÓDIGO NOVO — decore os 3 contextos:** `moneySpan()` só em conteúdo HTML de tela; `maskMoneyText()` em `textContent`, atributo (`title=`), `<option>` (o parser HTML **descarta** elementos filhos ali — `<span>` sumiria) e em valor que depois passa por `escapeHtml`/`svgText` (a tag seria escapada e apareceria literal); `asMoney()` cru só em documento/export. **Armadilha:** elemento atualizado via `.textContent` precisa de `maskMoneyText` **também no template inicial** (`#biTotal`, `#poSubtotal`/`#poDescontoView`/`#poTotalGeral`), senão o `<span>` some no primeiro `recompute()`. `formatCell()` NÃO usa moneySpan — `tableCell()` embrulha depois do escape. `savedProposalInternalHtml` é tratada como DOCUMENTO por precaução (vai para `#proposalPreview`, que é imprimível e não tem `.no-print`). Guarda automática: `scripts/tests/js/test_privacy_coverage.js` (23 asserções) falha se qualquer uma dessas regras for violada ou se um `.catch` vazio voltar. Suíte: 8/8.
>
> **v1.38.2 — Modo privacidade Etapa 2 (texto puro + inputs ad-hoc):** `showToast()` agora filtra a mensagem por `maskMoneyText()` — toast usa `textContent`, e o CSS de borrão não alcança pedaço de texto. **Correção junto:** `MONEY_TEXT_RE` era `/R\$\s?[\d.][\d.,]*/g` e o `[\d.,]*` guloso comia a pontuação da frase ("R$ 4.310,00." perdia o ponto; "R$ 1.000,00," perdia a vírgula); agora é `/R\$\s?\d(?:[\d.]*\d)?(?:,\d+)?/g` (o montante tem de terminar em dígito). Cinco inputs ad-hoc receberam a classe **`money-private`** (seletor novo no styles.css, borra até focar): `#biVu`, `#anfValor`, `#cnfValor`, `.pg-item-price`, `.po-i-vu` — **não** use `data-format="money"` nesses, o atributo liga formatação no blur e quebraria os handlers próprios. Varredura de `alert`/`confirm`/`prompt` com montante: **nenhum alvo** (as mensagens citam "valor" sem cifra); o `prompt` de reajuste de parcelas usa o montante como default EDITÁVEL e fica intacto. Teste novo `scripts/tests/js/test_privacy_mask.js` (13 asserções). **Falta a Etapa 3** (varredura dos ~169 `asMoney` — gaps já mapeados: Custo da Obra/execução, telas de IA, Cotações por material, Centros de Custo, Viabilidade Financeira, modais de recorrência/quitação, Agenda). Plano: `docs/superpowers/plans/2026-07-27-modo-privacidade.md`.
>
> **v1.38.1 — Impressão corrigida + rede de segurança de erros (Onda A: E1 e E2):** `cotacaoImprimir()` e `printViabilidadeReport()` chamavam **`openPrintDialog()`, que nunca existiu** — os dois botões lançavam `ReferenceError` e não imprimiam desde 27-28/06. Agora usam `printStandaloneDocument()` (a função real, já usada por contrato/RDO/pedido). **E1:** handlers globais `error` (com `capture:true`, para pegar recurso que não carrega) e `unhandledrejection` instalados **no topo do app.js** — a posição é deliberada: arquivo único, um erro no meio impede listeners do fim de existir. `reportGlobalError()` tem trava de reentrância (`errorHandlerRunning`), dedupe por assinatura (`ERROR_DEDUPE_WINDOW_MS` 60s), cooldown de toast (`ERROR_TOAST_COOLDOWN_MS` 10s), poda do Map `seenErrors`, e nunca propaga exceção nem exige `document.body`. **E2:** os 14 `.catch` vazios foram classificados — 12 acessórios usam `ignorarFalha(contexto)` (só console.warn) e 2 usam `avisarFalha(contexto, msg)` (console.error + toast) porque deixavam dado incompleto na tela: soltar itens ao remover etapa do orçamento e carregar itens da cotação. **Ao escrever código novo: nunca use `.catch(() => {})`** — escolha um dos dois helpers. Suíte nova `scripts/tests/js/test_error_handler.js` (11 asserções, extrai o bloco real do app.js via `vm`), rodada por `run-all.sh` junto com os testes PHP (6/6 blocos). Sem migration/schema/API.
>
> **v1.38.0 — Tema Dark Neutro:** frente exclusivamente visual. O dark em `styles.css` usa agora `#0f1115` (fundo), `#15181e` (sidebar/topbar), `#1b1f27` (cards/dialogs/drawers), `#222731` (inputs), `#292f3a` (hover), `#303744`/`#262c35` (bordas), `#f3f4f6`/`#a7afbd` (texto) e azul `#0a66c2`/`#1677d2`/`#4c9aff` para interação. Tokens funcionais: `--accent*`, `--surface-hover`, `--sidebar-surface`, `--topbar-surface`, `--focus-ring`, `--overlay`, `--chart-primary/secondary`; `--teal/--teal-2` são aliases de compatibilidade e resolvem para azul no dark. Verde fica semântico (`--green`: sucesso/concluído/recebido/lucro/positivo), vermelho para erro/prejuízo e âmbar para alerta. Gráficos de tela em `app.js` usam tokens CSS; negativos de barras horizontais ficam vermelhos. Plugin `plugins/seletividade/style.css` espelha os tokens, mas canvas e `@media print` continuam intocados. Não alterar `theme-init.js`, helpers/classes de privacidade (`privacyMode`, `money-blur`), PDFs/exports ou lógica. Spec/plano: `docs/superpowers/specs/2026-07-27-dark-theme-neutral.md` e `docs/superpowers/plans/2026-07-27-dark-theme-neutral.md`.
>
> **v1.37.0 — Lucro Gerencial × Caixa Real por período total:** o painel do Dashboard usa agora `getFilters().start/end` com intervalo inclusivo e quatro fallbacks (`inicial+final`; `inicial→hojeLocal()`; primeira data financeira→final; primeira data financeira→hoje). O recorte dimensional é próprio (`lucroCaixaMatchesDimensions`/`lucroCaixaScopedRows`) porque `applyFilters()` aceita qualquer data do registro e não serve para separar competência (`dueDate`) de caixa (`receivedDate`/`paidDate`). Cards consolidados: lucro, entradas, saídas, caixa líquido, diferença lucro×caixa e contas a receber/pagar abertas/vencidas (valor+quantidade). Gráfico: `lucroCaixaMonthlyRows` + `lucroCaixaEvolutionRows`, Mensal ou Acumulado; cards nunca mudam com o modo. Fórmula preservada: cancelados fora; `Parcial` integralmente aberto e fora do caixa efetivo (schema não tem valor parcial); título liquidado sem data efetiva usa `dueDate` como fallback histórico. Filtro cliente traz pagar pelas obras de `projects.clientId`. Sem endpoint/schema/migration novo; usa as coleções já carregadas pelo bootstrap. Privacidade preservada em cards/eixos/tooltips/alertas. Spec/plano: `docs/superpowers/specs/2026-07-27-lucro-caixa-periodo-total.md` e `docs/superpowers/plans/2026-07-27-lucro-caixa-periodo-total.md`.
>
> **v1.36.0 — Modo privacidade (Etapa 1 — dashboard):** botão de olho na topbar (`#privacyBtn` no index.html) e no dashboard (`#dashPrivacyToggle`) que borra montantes em R$ via classe `privacy-mode` no `<body>` + classe `money-blur` nos valores (CSS `filter: blur`). Estado em `localStorage` `finconta.privacy` (`privacyMode`/`applyPrivacyMode`/`togglePrivacyMode` no app.js, após `safeLocalSet`). Helpers: `moneySpan(v)` (span borrável p/ contexto HTML) e `maskMoneyText(t)` (texto puro — toasts/`<title>` SVG — vira "R$ •••"). Pontos centrais marcados: `kpi()` (format=true), `tableCell()` (embrulha DEPOIS do escape — não mover para `formatCell`, que é escapado), eixos/tooltips de `lineChart`/`groupedBarChart`/`horizontalBarChart`, `lucroCaixaPanel`/`lucroCaixaAlerts`, `dashboardAlerts`, execução (`dashboardExecutionWidgets`/`execTooltipHtml`). Regra: R$ ilegível; percentuais/contagens/datas e barras visíveis; exports/impressão de documentos intactos. SEM atalho de teclado (decisão do usuário). **Pendente: Etapa 2** (toasts via `maskMoneyText` no `showToast`, confirm/alert, inputs ad-hoc com classe `money-private`) e **Etapa 3** (varredura dos ~169 `asMoney` — plano `docs/superpowers/plans/2026-07-27-modo-privacidade.md`).
>
...
- **Cache busting:** ao mudar `app.js`/`styles.css`, incremente `?v=NNNN` em `index.html` (hoje `app.js?v=1802`, `styles.css?v=1802`) e a constante `APP_VERSION` no topo de `app.js`.
