...
- **Versão atual:** `v1.45.2` (2026-08-11). A versão fica em **dois lugares que devem andar juntos**: a constante `APP_VERSION`/`APP_VERSION_DATE` no topo de `app.js` (com `APP_CHANGELOG`) e o cabeçalho deste README. O painel "Versão" em Configurações lê de `APP_VERSION`.
- **Cache busting:** sempre que `app.js` ou `styles.css` mudarem, **incremente o `?v=NNNN`** das tags correspondentes em `index.html` (hoje `app.js?v=1817`, `styles.css?v=1817`). Sem isso o navegador serve a versão velha.
...
### v1.45.2 — 2026-08-11 · Fix: fotos em branco no PDF do RDO

A impressão era acionada antes de as fotos terminarem de carregar e o PDF saía sem nenhuma imagem (fotos grandes de celular perdiam a corrida sempre). O sistema agora espera cada imagem decodificar antes de abrir a tela de impressão — no RDO individual, no relatório semanal e nos demais documentos imprimíveis.
...
### v1.45.0 — 2026-08-11 · Diário de Obra — fotos HEIC do iPhone

O upload de fotos do RDO passa a aceitar `.heic`/`.heif` (formato padrão das fotos de iPhone), convertidas para JPEG **no servidor** no momento do envio — tela, PDF e relatório semanal seguem exibindo a foto como sempre.

- **Conversão no servidor** — CLI `heif-convert -q 85` (pacote `libheif-examples`); o `.heic` original é apagado e o banco guarda o `.jpg`. Sem o pacote instalado, o envio retorna 422 orientando `sudo apt install libheif-examples` (fotos JPG/PNG/WEBP não são afetadas).
- **Validação própria** — assinatura binária (`ftyp` + brand HEIF) no lugar da lista de MIME; HEIC falso/corrompido é rejeitado sem gravar nada. Burst/Live Photo usa o primeiro quadro.
- **Preview** — como o navegador não exibe HEIC, a foto pendente mostra o quadro "Prévia indisponível" até o envio.
- **Vídeos (MP4) seguem não aceitos** — decisão de produto: o diário registra imagens.
- Sem migration; `store_upload()` compartilhada intocada. Teste novo `test_rdo_heic.php` (16 asserções); suíte 22/22.
...
- O painel do Dashboard passou a usar as datas dos **filtros globais**: intervalo inclusivo; somente inicial vai até hoje; somente final começa no primeiro lançamento disponível; sem datas usa primeiro lançamento até hoje. O período efetivo aparece no cabeçalho.
- Os cards representam sempre o intervalo completo: lucro gerencial, entradas, saídas, caixa líquido, diferença lucro × caixa e contas a receber/pagar abertas e vencidas, com valor e quantidade.
- O gráfico é o detalhamento mensal do mesmo intervalo e ganhou as visões **Mensal** e **Acumulado**; no acumulado cada mês soma os anteriores desde o início do recorte.
- Fórmula preservada: competência por `dueDate` (exceto cancelados); caixa por `receivedDate`/`paidDate`, com fallback para `dueDate` quando um título liquidado não tem data efetiva. `Parcial` permanece integralmente em aberto e fora do caixa efetivo porque o schema atual não possui valor parcial liquidado.
- Filtros de obra, cliente e centro de custo passam a recortar diretamente o painel. No cliente, despesas entram pelas obras vinculadas ao cliente; `accounts_payable` não possui `clientId` direto.
- Sem endpoint ou schema novo: `accounts_receivable` e `accounts_payable` já chegam numa carga do bootstrap; o frontend agrega os meses sem consultas repetidas. Spec e plano: `docs/superpowers/specs/2026-07-27-lucro-caixa-periodo-total.md` e `docs/superpowers/plans/2026-07-27-lucro-caixa-periodo-total.md`.
- Modo privacidade preservado em cards, alertas, eixos e tooltips; datas e quantidades continuam visíveis.
...
- Filtros por período, cliente, obra/projeto, status da obra, centro de custo e categoria. O painel Lucro Gerencial × Caixa Real usa o intervalo total desses filtros e mostra evolução mensal ou acumulada.
