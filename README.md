...
O botão "Gerar PDF" do RDO deixou de abrir a tela de impressão e passa a baixar um PDF de verdade (`RDO-<nº>-<obra>-<data>.pdf`), gerado no servidor com dompdf: cabeçalho da empresa com logo, todos os blocos do diário, fotos com legenda e assinaturas com CPF. Requer `composer require dompdf/dompdf` no servidor (sem a biblioteca, o botão orienta a instalação). O relatório semanal continua no fluxo de impressão.
